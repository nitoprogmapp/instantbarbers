...
function BarberDashboard() {
  ...
  const loadBookings = async () => {
    try {
      console.log("📡 Fetching bookings for barber:", barberId);

      const response = await fetch(
        `http://127.0.0.1:8000/bookings/barber/${barberId}`
      );
  ...
  const acceptBooking = async (bookingId) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/bookings/${bookingId}/accept`,
        {
          method: "PUT",
        }
      );
  ...
  useEffect(() => {
    // 🔥 IMPORTANTE: convertir a número
    const savedBarberId = localStorage.getItem("barberId");

    if (savedBarberId) {
      setBarberId(parseInt(savedBarberId)); // 💥 FIX CLAVE
    } else {
      setBarberId(1); // fallback seguro
    }
  }, []);
    ...
    if (!firstComparisonDoneRef.current) {
      ...
      playNotificationSound().then((played) => {
        if (played) {
          ...
          setMessage(`New booking detected (sound blocked). ID: ${newIds.join(", ")}`);
        }
      });
